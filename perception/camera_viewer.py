...
class CameraViewer(Node):
    def __init__(self):
        ...
        self.rgb_sub = self.create_subscription(Image, CAMERA_RGB_TOPIC, self._on_image, sensor_qos)
    ...
    def _on_image(self, msg: Image):
        ...
        try:
            if msg.encoding == "rgb8":
                ...
                raw = np.frombuffer(msg.data, dtype=np.uint8).reshape(msg.height, msg.width, 2).copy()
                bgr = cv2.cvtColor(raw, cv2.COLOR_YUV2BGR_UYVY)
                #cv2.imwrite(os.path.join(DEBUG_FRAMES, f"yuv_debug_{self.frame_count:06d}.png"), bgr)
            else:
